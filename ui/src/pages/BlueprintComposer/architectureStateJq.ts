/**
 * The jq the composition detail page's `composition-architecture` RESTAction runs (portal S11c,
 * helm/portal/templates/restaction.composition-architecture.yaml at portal f73ad15), copied VERBATIM
 * so the controller's projected state and the page's state are one computation. Change it there and
 * here together: projectionCompile.test.ts pins each copy's SHA-256, taken from the portal file.
 *
 * The top filter reads `.arch` (the graph, from the arch step), `.comp[0].managed` and `.objs`, plus
 * the steps' error lists under `archErr` / `compErr` / `objErr`.
 */

/** The `arch` step's filter: the rendered graph of THIS composition, or null. */
export const S11_ARCH_STEP_FILTER = `((.extras.compositionName // .extras.name) // "") as $n
...
/** The `objs` step's filter: each object reduced to what the top filter reads. */
export const S11_OBJECT_FILTER = `[ (.objs // .) | select(type == "object" and (.metadata.name // "") != "")
  | { apiVersion: (.apiVersion // ""),
      metadata: { name: .metadata.name, namespace: (.metadata.namespace // ""), creationTimestamp: (.metadata.creationTimestamp // "") },
      status: (.status // {}) } ]
`

/** The top filter: the graph + the objects → architectureState. */
export const S11_TOP_FILTER = `# Input: snowplow's resolve dict — the request extras at the top level, each stage's (step-filtered)
