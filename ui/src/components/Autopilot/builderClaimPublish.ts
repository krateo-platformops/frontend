...
export const buildClaimPublish = async (args: {
  ...
  // A SEEDED repository is named for its one chart (seededRepoProblem). The publish form refuses
  // another name as it is typed; this is the same rule where every builder's claim is built, so a
  // caller that skipped the form — a headless run, a model-emitted repo — cannot get past it either.
  const seeded = args.sourceUrl ? seededRepoProblem(args.slug, target.repo) : null
  if (seeded) {
    return { branch: `builder/${args.slug}`, compiled: { denial: `denied — ${seeded}`, ops: null }, deepLink: null }
  }
