...
> because no control anywhere could publish a *held* draft. Both were accurate about different
> things, which is precisely the self-contradiction this section existed to remove:
>
> - the builder FORMS publish what the form itself collected — `form.compose-page` POSTs a
>   `BuilderPublish` claim, and that has always been a human-pressable publish control;
> - none of them could publish the draft in `blueprintDraftStore`, so anything Autopilot authored,
>   or anything richer than a flat page of existing widgets, was reachable only through the rail.
>
> **The second is now closed** (frontend#271): `/portal-builder/compose` mounts the preview surface
...
>
> *(Recorded 2026-09-15, when it was accurate. The capability half closed on 2026-09-16; the
> scoping below stands as the reason it took a page rather than another form.)*
>
